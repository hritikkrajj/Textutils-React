...
export default function TextForm(props) {
  ...
  return (
    ...
    <div className="container"style={{color: props.mode ==='dark'?'white':'#31366f'}}>
        ...
        <h1> {props.heading}</h1>
            <div className="mb-3">
            <textarea className="form-control"value={text} onChange={handleonchange} style={{backgroundColor: props.mode ==='dark'?'grey':'white',color:props.mode ==='dark'?'white':'#31366f'}} id="myBox" rows="8"></textarea>
      </div>
      ...
      <button className="btn btn-primary mx-2" onClick={handleCleanClick}>Clean Text</button>
      <button className="btn btn-primary mx-2" onClick={handleCopy}>Copy text</button>
      <button className="btn btn-primary mx-2" onClick={handleExtraSpaces}>Remove extrs spaces</button>
    </div>
